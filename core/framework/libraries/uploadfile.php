...
class UploadFile{
	...
	private $allow_type=array('gif','jpg','jpeg','bmp','png','swf','tbi');
        ...
        允许上传的附件文件类型
         *          */
        private $allow_attachment_type=array('doc','xls','ppt','docx','xlsx','pptx');
                        
	/**
	 * 允许的最大文件大小，单位为KB
	 */
	private $max_size = '3072';
	/**
	 * 改变后的图片宽度
	 */
...
//		$this->setErrorFileName($this->upload_file['tmp_name']);
	 ...
	 * @return bool
	 */
	public function upfileAttachment($field){
		...
		//验证文件格式是否为系统允许
		if(!in_array($this->ext,$this->allow_attachment_type)){
			$error = Language::get('image_allow_ext_is').implode(',',$this->allow_attachment_type);
			$this->setError($error);
			return false;
		}
