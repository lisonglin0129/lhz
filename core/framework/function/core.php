 ...
 */
function setShopCookie($name, $value, $expire='3600', $path='', $domain='', $secure=false){
	if (empty($path)) $path = '/';
	if (empty($domain)) $domain = SUBDOMAIN_SUFFIX ? SUBDOMAIN_SUFFIX : '';
	$name = defined('COOKIE_PRE') ? $name : strtoupper(substr(md5(MD5_KEY),0,4)).'_'.$name;
...
function write_file($filepath, $data, $mode = null)
{
    ...
    $data = "<?php defined('ShopMall') or exit('Access Invalid!'); return ".$data.";";
