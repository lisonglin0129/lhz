...
final class Base{
	...
	public static function autoload($class){
		...
		}elseif (substr($orclass,0,8) == 'PHPExcel' ){
			$pcls = str_replace('_','/',$orclass); 
			if (!@include_once(BASE_CORE_PATH.'/framework/function/'.$pcls.'.php')){
				exit("Class Error: {$class}.isn't exists!");
			}
		}else{
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			header('location: http://www.laohanzheng.com');exit();
		}else{
