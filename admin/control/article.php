...
class articleControl extends SystemControl {
    ...
    public function article_addOp() {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                $insert_array['attachment'] = trim($_POST['attachment']);
                $insert_array['attachment_extension'] = trim($_POST['attachment_extension']);
    ...
    public function article_editOp() {
        ...
        if (chksubmit()) {
            ...
            if ($error != '') {
                ...
                $update_array['attachment'] = trim($_POST['attachment']);
                $update_array['attachment_extension'] = trim($_POST['attachment_extension']);
     ...
     * 上传附件
     */
    public function article_attachment_uploadOp() {
        $upload = new UploadFile();
        $upload->set('max_size',10240);
        $upload->set('default_dir', ATTACH_ARTICLE);
      
        $result = $upload->upfileAttachment('fileupload');
        ...
        if ($result) {
            ...
            if ($filename) {
                ...
                $data['attachment_extension'] = $extension;
                /**
                 * 整理为json格式
                 */
                $output = json_encode($data);
