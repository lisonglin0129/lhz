	...
	public function navigation_addOp(){
		...
		if (chksubmit()){
			...
			switch ($_POST['nav_type']){
				...
				*/
				case '4':
					$obj_validate->setValidate(array("input"=>$_POST["special_id"], "require"=>"true", "message"=>$lang['navigation_add_special_null']));
					break;
 						/**
				 * 专区
				 */
				case '5':
					$obj_validate->setValidate(array("input"=>$_POST["spgoods_id"], "require"=>"true", "message"=>$lang['navigation_add_activity_null']));
					break;
			...
			if ($error != ''){
				...
				switch ($_POST['nav_type']){
					...
					case '3':
						...
						break;
					case '4':
						$insert_array['item_id'] = intval($_POST['special_id']);
						break;
 				   case '5':
						$insert_array['item_id'] = intval($_POST['spgoods_id']);
						break;
		...
		$spgoods	= Model('spgoods');
		$spgoods_list	= $spgoods->getList(array('opening'=>true,'order'=>'spgoods.activity_sort'));
		 
		/**
		 * 频道
		 */
		//$special	= Model('cms_special');
		//$special_list	= $special->getShopList(array('special_state'=>2));
		
		$special	= Model('web_channel');
		$special_list	= $special->getChannelList(array('channel_show'=>1));
		Tpl::output('special_list',$special_list);
		Tpl::output('activity_list',$activity_list);
		Tpl::output('spgoods_list',$spgoods_list);
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				switch ($_POST['nav_type']){
					...
					*/
					case '4':
						$update_array['item_id'] = intval($_POST['special_id']);
						break;
					case '5':
						$update_array['item_id'] = intval($_POST['spgoods_id']);
						break;
