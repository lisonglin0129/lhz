	...
	public function templateaddOp(){
		...
		if(chksubmit()){
			...
			if ($error){
				...
				$insert_arr['voucher_t_store_id'] = 0;
				$insert_arr['voucher_t_storename'] = '老汉正超市供货平台';
				$insert_arr['voucher_t_sc_id'] = intval($_POST['sc_id']);
				
				$insert_arr['voucher_t_sc_name'] = $gc_name;
				$insert_arr['voucher_t_creator_id'] = 'admin';
				...
				if (!empty($_FILES['_pic']['name'])){
					...
					$upload->set('default_dir',	ATTACH_VOUCHER.DS.'0');
				...
				if($rs){
					showDialog(Language::get('nc_common_save_succ'),'index.php?act=voucher&op=templateadd','succ');
				}else{
					showDialog(Language::get('nc_common_save_fail'),'index.php?act=voucher&op=templateadd','error');
				}
			}
		}else{
			//经营分类
			...
			Tpl::output('show_page',$model->showpage(2));
			$this->show_menu('voucher','templateadd');
		...
		switch ($menu_type) {
			case 'voucher':
				...
				4=>array('menu_key'=>'setting','menu_name'=>Language::get('admin_voucher_setting'),	'menu_url'=>'index.php?act=voucher&op=setting'),
					7=>array('menu_key'=>'templateadd','menu_name'=>'添加平台代金券', 'menu_url'=>'index.php?act=voucher&op=templateadd'),
				);
				break;
			case 'priceedit':
				$menu_array = array(
