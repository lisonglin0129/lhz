...
class memberControl extends SystemControl{
	...
	public function member_editOp(){
		...
		if (chksubmit()){
			...
			array("input"=>$_POST["member_mobile"], "require"=>"true", 'validator'=>'Email', "message"=>$lang['member_edit_valid_email']),
			);
			...
			if ($error != ''){
				...
				$update_array['is_check'] 			= $_POST['is_check'];
				$update_array['checkremark'] 			= $_POST['checkremark'];
				...
				$update_array['licence_number'] 		= $_POST['licence_number'];
				$update_array['business_licence_number'] 		= $_POST['business_licence_number'];
				$update_array['contact_phone'] 		= $_POST['contact_phone'];
				$update_array['member_address_detail'] 		= $_POST['member_address_detail'];
			 
				if (!empty($_FILES['licence_pic']['name'])){
					$upload = new UploadFile();
					$upload->set('default_dir',ATTACH_STORE_JOININ);
					$result = $upload->upfile('licence_pic');
					if ($result){
						$_POST['licence_number_electronic'] = $upload->file_name;
					}else {
						showMessage($upload->error,'','','error');
					}
					$update_array['licence_number_electronic'] = $_POST['licence_number_electronic'];
				}
				
				if (!empty($_FILES['business_licence_pic']['name'])){
					$upload = new UploadFile();
					$upload->set('default_dir',ATTACH_STORE_JOININ);
					$result = $upload->upfile('business_licence_pic');
					if ($result){
						$_POST['business_number_electronic'] = $upload->file_name;
					}else {
						showMessage($upload->error,'','','error');
					}
					$update_array['business_licence_number_electronic'] = $_POST['business_number_electronic'];
				}
