...
class  spgoodsControl extends SystemControl{
	public function __construct(){
		parent::__construct();
		Language::read('spgoods');
	}
	/**
	 * 专区列表
	 */
	...
	public function activityOp(){
		$activity	= Model('spgoods');
		//条件
		$condition_arr = array();
	//	$condition_arr['activity_type'] = '1';//只显示商品专区
		//状态
	...
	public function newOp(){
		...
		if($result){
			...
			showMessage(Language::get('nc_common_op_succ'),'index.php?act=spgoods&op=spgoods');
		}else{
			//添加失败则删除刚刚上传的图片,节省空间资源
			@unlink(BASE_UPLOAD_PATH.DS.ATTACH_spgoods.DS.$upload->file_name);
	...
	public function ajaxOp(){
		if(in_array($_GET['branch'],array('activity_title','activity_sort'))){
			...
			$activity_detail =Model('spgoods_detail');
			$update_array = array();
			switch ($_GET['branch']){
				/**
				 * 排序
	...
	public function delOp(){
		...
		if (empty($activity_list)){//没有符合条件的专区信息直接返回成功信息
	...
	public function editOp(){
		...
		if($result){
			if($_FILES['activity_banner']['name']!=''){
				@unlink(BASE_UPLOAD_PATH.DS.ATTACH_spgoods.DS.$row['activity_banner']);
			}
			$this->log(L('nc_edit,activity_index').'[ID:'.$_POST['activity_id'].']',null);
			showMessage(Language::get('nc_common_save_succ'),'index.php?act=spgoods&op=spgoods');
		}else{
			if($_FILES['activity_banner']['name']!=''){
				@unlink(BASE_UPLOAD_PATH.DS.ATTACH_spgoods.DS.$upload->file_name);
			}
	...
	public function detailOp(){
		...
		$condition_arr['order'] = 'spgoods_detail.activity_detail_state asc,spgoods_detail.activity_detail_sort asc';

		$page	= new Page();
		$page->setEachNum(10);
		$page->setStyle('admin');
		$activitydetail_model	=Model('spgoods_detail');
		$list	= $activitydetail_model->getList($condition_arr,$page);
		//输出到模板
		Tpl::output('show_page',$page->show());
		Tpl::output('list',$list);
		Tpl::showpage('spgoods_detail.index');
	}
