...
class storeControl extends SystemControl{
	...
	public function store_editOp(){
		...
		if (chksubmit()){
			...
			$update_array['store_creditcard'] = trim($_POST['store_creditcard']);//保障服务-信用卡支付
			$update_array['store_zhping'] = trim($_POST['store_zhping']);//保障服务-正品保证
			$update_array['store_tuihuo'] = trim($_POST['store_tuihuo']);//保障服务-退货承诺
			$update_array['store_debt'] = trim($_POST['store_debt']);//保障服务-赊账
    ...
    public function store_bind_class_addOp() {
		...
		$condi = array();
 		$condi['class_1'] = $class_1;
		$condi['state'] = 1;
		if(!empty($class_2)) {
			$condi['class_2'] = $class_2;
		}
		if(!empty($class_3)) {
			$condi['class_3'] = $class_3;
		}
		$model_goods_class = Model('goods_class');
		$gc_list = $model_goods_class->getGoodsClassThreeList($condi);
		$result=array();
		foreach($gc_list as $val )
		{
			...
			$param['state'] = 1;
			$param['class_2'] = $val['gc_parent_id'];
			$param['class_3'] = $val['gc_id'];
			// 检查类目是否已经存在
		...
		if(!empty($result)) {
			$this->log('添加店铺经营类目，类目编号:'.var_dump($result).',店铺编号:'.$store_id);
...
/*         if($model_seller->isSellerExist(array('seller_name' => $joinin_detail['seller_name']))) {
        ...
        if($_POST['verify_type'] === 'pass') {
			...
			echo $joinin_detail['sc_id'];
			if($joinin_detail['sc_id']==''){
				$shop_array['sc_id'] = 11;
			}else{
