...
class SystemControl{
	...
	protected function __construct(){
		$this->root_path = str_replace('admin/control/control.php', '', str_replace('\\', '/', __FILE__));
		require_once $this->root_path.'core/framework/libraries/view.class.php';
		Language::read('common,layout');
		$this->tmp = new View();
		$this->tmp->template_dir =  $viewPath.'admin';
		$this->tmp->cache_dir = $viewPath.'data/cache/tpl';
		$this->tmp->direct_output = true;
		$this->tmp->force_compile = true;
		/**
		 * 验证用户是否登录
	...
	{
		$this->tmp->assign($key, $value);
	}
	protected function display($str = '')
	{
		$this->tmp->display($str);
	}
	/**
	 * 取得当前管理员信息
	 *
