...
class CMBC {
	...
	public function reBill()
	{
		...
		$simpleXML->Body->SerialNumber= '10000020'.time();
		$simpleXML->Body->OrderNo=  $this->order_info['return_sn'];
		...
		$simpleXML->Body->BankAccount->BranchName= $this->order_info['BranchName'];
		$simpleXML->Body->BankAccount->Province = $this->order_info['Province'];
		$simpleXML->Body->BankAccount->City = $this->order_info['City'];
	...
	public function refund()
	{
		...
		//--绑定参数
		$simpleXML->Body->InstitutionID= $this->pay_data['payment_config']['institutionID'];
		$simpleXML->Body->SerialNumber= '1100020'.time();
		//$simpleXML->Body->SerialNumber= $this->order_info['pay_sn'];
		$simpleXML->Body->OrderNo= !empty($this->order_info['return_sn']) ? $this->order_info['return_sn']:$this->order_info['order_sn'];
	...
	private function get_web_content( $curl_data )
	{
		...
		$ch      = curl_init($this->pay_data['payment_config']['TXURL']);
