...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      <h3>买家赊账管理</h3>
      <ul class="tab-base">
        <li><a href="index.php?act=<?php echo $_GET['act'];?>&op=credit_manage"><span>买家赊账催缴</span></a></li>
        <li><a href="JavaScript:void(0);" class="current"><span>确认收款</span></a></li>
      </ul>
    </div>
  </div>
  <div class="fixed-empty"></div>
  <form method="post" name="form1" id="form1" action="index.php?act=<?php echo $_GET['act'];?>&op=receive_pay&ban_id=<?php echo intval($_GET['ban_id']);?>">
    <input type="hidden" name="form_submit" value="ok" />
    <input type="hidden" value="<?php echo getReferer();?>" name="ref_url">
    <table class="table tb-type2">
      <tbody>
         
        <?php if ($_GET['act'] == 'credit') { ?>
        <tr class="noborder">
          <td colspan="2" class="required"><label for="site_name">支付单号<?php echo $lang['nc_colon'];?></label></td>
</tr>
<tr class="noborder">
          <td class="vatop rowform"><?php echo $output['order_info']['balance_sn'];?></td>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><?php echo $output['order_info']['debt_amount'];?></td>
          <td class="vatop tips"></td>
        </tr>
        <tr class="noborder">
