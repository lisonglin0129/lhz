...
<div class="page">
  <table class="table tb-type2 order">
      ...
      <tr>
        <td colspan="2"><ul>
            ...
            <li><strong><?php echo $lang['order_total_price'];?>:</strong><span class="red_common"><?php echo $lang['currency'].$output['order_info']['order_amount'];?> </span>
			  <?php if( $output['order_info']['debt_amount']>0){?> （赊账：<?php echo$output['order_info']['debt_amount'];?> ） <?php }?>
    ...
    <!-- S 促销信息 -->
      ...
      <tr>
        ...
        使用了面额为 <?php echo $lang['nc_colon'];?> <?php echo $output['order_info']['extend_order_common']['voucher_price'];?> 元的消费券，
  ...
  <?php if(is_array($output['oclog_list']) and !empty($output['oclog_list'])) { ?>
      <tr>
      	<th>赊账操作记录</th>
      </tr>
      <?php foreach($output['oclog_list'] as $val) { ?>
	<tr>
        <td>发生时间<?php echo $lang['nc_colon'];?><?php echo date("Y-m-d H:i:s",$val['lg_add_time']); ?>&emsp;&emsp;操作人<?php echo $lang['nc_colon'];?><?php echo $val['lg_admin_name'];?>&emsp;&emsp;赊账金额<?php echo $lang['nc_colon'];?><?php echo $lang['currency'];?><?php echo $val['lg_amount']; ?>&emsp;备注<?php echo $lang['nc_colon'];?><?php echo $val['lg_desc'];?></td>
      </tr>
    <?php } ?>
	  <?php } ?>
