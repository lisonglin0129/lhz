        ...
        <tr class="noborder">
          <td colspan="2"><ul id="thumbnails" class="thumblists">
              ...
              <?php } ?>
                        ...
                        <tr>
                    <td colspan="2" class="required">上传附件:</td>
                </tr>
                <tr class="noborder">
            <input type="hidden" name="attachment" id="attachment" value="<?php echo $output['article_array']['attachment'];?>"/>
            <input type="hidden" name="attachment_extension" id="attachment_extension" value="<?php echo $output['article_array']['attachment_extension'];?>"/>
            <td colspan="3" id="divComUploadContainer"><input type="file" multiple="multiple" id="fileupload1" name="fileupload" /></td>
            </tr>
            <tr>
                <td colspan="2" class="required">已传附件:</td>
            </tr>
            <tr class="noborder">
                <td colspan="2">
                    <ul id="thumbnailsfiles" class="thumblists"> 
                        <li class="picture"><?php echo $output['article_array']['attachment'];?></li>
                    </ul> 
                    <div class="tdare"> 
                    </div></td> 
            </tr>
...
$(document).ready(function(){
    ...
    $('#fileupload').each(function(){
        ...
        $('#fileupload1').fileupload({
                                                    dataType: 'json',
                                                    url: 'index.php?act=article&op=article_attachment_upload',
                                                    done: function (e, data) {
                                                        if (data != 'error') {
                                                            set_atteachment(data.result);
                                                        }
                                                    }
                                                });
});
                                     ...
                                     //设置隐藏域的值
                                            function set_atteachment(obj) {
                                                $('#attachment').val(obj.attachment);
                                                $('#attachment_extension').val(obj.attachment_extension);
                                                $('#thumbnailsfiles').html('<li  class="picture" >  ' + obj.attachment + ' </li>')
                                            }
