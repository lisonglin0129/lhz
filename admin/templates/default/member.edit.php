    ...
    <table class="table tb-type2">
      <tbody>
        ...
        <tr class="noborder">
          ...
          <tr>
          <td colspan="2" class="required"><label class="validation"  for="member_mobile">认证手机号码:</label></td>
        </tr>
        <tr class="noborder">
		...
		<tr>
          <td colspan="2" class="required"><label for="licence_number">工商注册信息:</label></td>
        </tr>
        <tr class="noborder">
          ...
          <td class="vatop rowform">
			<span class="business_type-file-show">
			<img class="show_image" src="<?php echo ADMIN_TEMPLATES_URL;?>/images/preview.png">
			<div class="business_type-file-preview" style="display: none;"><img src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_STORE_JOININ; ?>/<?php echo $output['member_array']['business_licence_number_electronic'];?>" id="'business_view_img"></div>
			</span>
            <span class="type-file-box">
              <input type='text' name='business_number_electronic' id='business_number_electronic' class='business_type-file-text' />
              <input type='button' name='business_button' id='business_licence_button' value='' class='business_type-file-button' />
              <input name="business_licence_pic" type="file" class="business_type-file-file" id="business_licence_pic" size="30" hidefocus="true" />
            </span>            
            </td>
		...
		<tr>
          <td colspan="2" class="required"><label for="licence_number">身份证号码:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php echo $output['member_array']['licence_number'];?>" id="licence_number" name="licence_number" class="txt"></td>
          <td class="vatop tips"></td>
        </tr>
		  <tr class="noborder">
		   <td class="vatop rowform"><span class="type-file-show"><img class="show_image" src="<?php echo ADMIN_TEMPLATES_URL;?>/images/preview.png">
            <div class="type-file-preview"><img src="<?php echo UPLOAD_SITE_URL.'/'.ATTACH_STORE_JOININ; ?>/<?php echo $output['member_array']['licence_number_electronic'];?>" id="licence_view_img"></div>
            </span><span class="type-file-box">
			<input type='text' name='licence_number_electronic' id='licence_number_electronic' class='licence_type-file-text' />
			<input type='button' name='licence_button' id='licence_button' value='' class='licence_type-file-button' />
            <input name="licence_pic" type="file" class="licence_type-file-file" id="licence_pic" size="30" hidefocus="true" nc_type="change_licence_number_electronic">
            </span></td>
          <td class="vatop tips"><?php echo $lang['member_edit_support']?>gif,jpg,jpeg,png</td>
		 </tr>
        <!-- 显示所在地-->
        ...
        <tr>
          <td colspan="2" class="required"><label for="member_address_detail">超市详细地址:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php echo $output['member_array']['member_address_detail'];?>" id="member_address_detail" name="member_address_detail" class="txt"></td>
          <td class="vatop tips"></td>
        </tr>
          <tr>
          <td colspan="2" class="required"><label for="contact_phone">常用联系电话:</label></td>
        </tr>
        <tr class="noborder">
          <td class="vatop rowform"><input type="text" value="<?php echo $output['member_array']['contact_phone'];?>" id="contact_phone" name="contact_phone" class="txt"></td>
          <td class="vatop tips"></td>
        </tr>
        ...
        <tr class="noborder">
		 ...
		 <tr>
          <td colspan="2" class="required"><label>会员资料审核</label></td>
        </tr>
	
        <tr class="noborder">
          <td class="vatop rowform onoff">
			<label for="is_check1" class="cb-enable <?php if($output['member_array']['is_check'] == '2'){ ?>selected<?php } ?>" ><span>审核通过</span></label>
            <label for="is_check2" class="cb-disable <?php if($output['member_array']['is_check'] == '1'){ ?>selected<?php } ?>" ><span>不通过</span></label>
            <input id="is_check1" name="is_check" <?php if($output['member_array']['is_check'] == '2'){ ?>checked="checked"<?php } ?>  value="2" type="radio">
            <input id="is_check2" name="is_check" <?php if($output['member_array']['is_check'] == '1'){ ?>checked="checked"<?php } ?> value="1" type="radio"></td>
          <td class="vatop tips"></td>
        </tr>
		  <tr>
          <td colspan="2" class="required"><label>会员资料审核说明</label></td>
        </tr>
		   <tr class="noborder">
          <td class="vatop rowform"><textarea  type="text"   id="checkremark" name="checkremark" class="tarea"  rows="6" ><?php echo $output['member_array']['checkremark'];?> </textarea></td>
          <td class="vatop tips">对于审核资料不合格的请详细说明，以便提醒会员修改</td>
        </tr>
        <tr>
...
$(function(){
	...
	$("#licence_pic").change(function(){
		$("#licence_number_electronic").val($(this).val());
		
	});
	
	$("#business_licence_pic").change(function(){
		$("#business_number_electronic").val($(this).val());
		 
	});
	
	 
	// 上传图片类型
$('input[class="business_type-file-file"]').change(function(){
	...
	$('input[class="licence_type-file-file"]').change(function(){
	var filepatd=$(this).val();
	var extStart=filepatd.lastIndexOf(".");
	var ext=filepatd.substring(extStart,filepatd.lengtd).toUpperCase();		
		if(ext!=".PNG"&&ext!=".GIF"&&ext!=".JPG"&&ext!=".JPEG"){
	...
	function uploadChange(){
		...
		var uploadpath='<?php echo ATTACH_AVATAR;?>';
		var newfilename='<?php echo 'avatar_'.$_GET['member_id'].'.jpg';?>';
		ajaxFileUpload($(this).attr("name"),uploadpath,newfilename);
	};
	
	function ajaxFileUpload(InputName,UploadPath,newfilename)
	{
		$.ajaxFileUpload
		(
			{
				url:'index.php?act=common&op=pic_upload&form_submit=ok&uploadpath='+UploadPath,
				secureuri:false,
				...
				{
					if (data.status == 1){
						ajax_form('cutpic','<?php echo $lang['nc_cut'];?>','index.php?act=common&op=pic_cut&type=member&x=120&y=120&resize=1&ratio=1&filename=<?php echo UPLOAD_SITE_URL.'/';?>'+UploadPath+'/'+newfilename+'&url='+data.url,690);
					}else{
						alert(data.msg);
					}
