...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?act=spgoods&op=spgoods" ><span><?php echo $lang['nc_manage'];?></span></a></li>
        <li><a href="index.php?act=spgoods&op=new" ><span><?php echo $lang['nc_new'];?></span></a></li>
        ...
        <tr class="hover">
          ...
          <td class="w150 align-center">
          	<?php if($v['activity_detail_state']!='1'){?>
            	<a href="index.php?act=spgoods&op=deal&activity_detail_id=<?php echo $v['activity_detail_id'];?>&state=1"><?php echo $lang['activity_detail_index_pass'];?></a>
            <?php }?>
            <?php if($v['activity_detail_state']=='0'){?>
            	&nbsp;|&nbsp;
            <?php }?>
            <?php if($v['activity_detail_state']!='2'){?>
            	<a href="index.php?act=spgoods&op=deal&activity_detail_id=<?php echo $v['activity_detail_id'];?>&state=2"><?php echo $lang['activity_detail_index_refuse'];?></a>
            <?php }?>
            <?php if ($v['activity_detail_state']=='0' || $v['activity_detail_state']=='2'){?>
            	&nbsp;|&nbsp;<a href="javascript:void(0)" onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){location.href='index.php?act=spgoods&op=del_detail&activity_detail_id=<?php echo $v['activity_detail_id'];?>';}"><?php echo $lang['nc_del'];?></a></td>
            <?php }?>
        </tr>
        <?php } ?>
        <?php }else { ?>
...
function submit_form(op){
	if(op=='del'){
		...
		$('#listform').attr('action','index.php?act=spgoods&op=del_detail');
	}else if(op=='pass'){
		if(!confirm('<?php echo $lang['activity_detail_index_pass_all'];?>')){
			return false;
		}
		$('#listform').attr('action','index.php?act=spgoods&op=deal&state=1');
	}else if(op=='refuse'){
		if(!confirm('<?php echo $lang['activity_detail_index_refuse_all'];?>')){
			return false;
		}
		$('#listform').attr('action','index.php?act=spgoods&op=deal&state=2');
	}
	$('#listform').submit();
}
</script>
