      ...
      <tbody>
        ...
        <tr class="noborder">
          <td class="vatop rowform"><ul class="nofloat">
			 ...
			 <li class="left w100pre"><span class="radio">
                <input type="radio" <?php if($output['navigation_array']['nav_type'] == '4'){ ?>checked="checked"<?php } ?> value="4" name="nav_type" id="'special'" onclick="showType('special');">
                <label for="special">频道</label>
                </span>
                <select name="special_id" id="special_id" style="display: none;">
                  <?php if(is_array($output['special_list'])){ ?>
                  <?php foreach($output['special_list'] as $k => $v){ ?>
                  <option <?php if($output['navigation_array']['item_id'] == $v['channel_id']){ ?>selected="selected"<?php } ?> value="<?php echo $v['channel_id'];?>"><?php echo $v['channel_name'];?></option>
                  <?php } ?>
                  <?php } ?>
                </select>
              </li>
			  <li class="left w100pre"><span class="radio">
                <input type="radio" <?php if($output['navigation_array']['nav_type'] == '5'){ ?>checked="checked"<?php } ?> value="5" name="nav_type" id="spgoods" onclick="showType('spgoods');">
                <label for="spgoods"> 专区</label>
                </span>
                <select name="spgoods_id" id="spgoods_id" style="display: none;">
                  <?php if(is_array($output['spgoods_list'])){ ?>
                  <?php foreach($output['spgoods_list'] as $k => $v){ ?>
                  <option <?php if($output['navigation_array']['item_id'] == $v['activity_id']){ ?>selected="selected"<?php } ?> value="<?php echo $v['activity_id'];?>"><?php echo $v['activity_title'];?></option>
                  <?php } ?>
                  <?php } ?>
                </select>
...
$(document).ready(function(){
	...
	showType('special');
	<?php } ?>
		<?php if($output['navigation_array']['nav_type'] == '5'){ ?>
	showType('spgoods');
	<?php } ?>
});
...
function showType(type){
	...
	$('#special_id').css('display','none');
	$('#spgoods_id').css('display','none');
 	if(type == 'diy'){
