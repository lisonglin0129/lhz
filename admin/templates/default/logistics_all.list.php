...
<div class="page">
  ...
  <form method="get" action="index.php" name="formSearch" id="formSearch">
    ...
    <input type="hidden" name="op" value="logistics_all" />
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
        <th><select name="type">
            <option value="logis_sn" <?php if($_GET['type'] == 'logis_sn'){?>selected<?php }?>>配送单号</option>
        ...
        <td><input type="text" class="text" name="key" value="<?php echo trim($_GET['key']); ?>" /></td>
          <th><label for="add_time_from"><?php echo '生成时间';?></label></th>
    ...
    <tbody>
      ...
      <?php foreach ($output['logistics_list'] as $key => $val) { ?>
	    ...
	    <td><?php echo $val['order_amount']; ?></td>
           ...
           <td class="align-center"><a href="index.php?act=logistics&op=edit_logistics&logis_id=<?php echo $val['logis_id']; ?>"> 查看 </a>
		<?php if  ($val['order_state']=='10') {?>
		<a href="index.php?act=logistics&op=del_logistics&logis_id=<?php echo $val['logis_id']; ?>">删除 </a>
		  <?php } ?>
		</td>
      </tr>
      <?php } ?>
